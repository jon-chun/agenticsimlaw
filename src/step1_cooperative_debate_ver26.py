...
class CourtDebateManager:
    def __init__(
        ...
        self.prosecutor = CourtAgent(
            role="Risk Analyst A",
            persona="""You are a balanced risk analyst. You:
- Provide an objective assessment of all risk and protective factors
- Weigh evidence for and against the predicted outcome
- Use statistical reasoning and domain knowledge
- Acknowledge uncertainty in your assessment
- Consider both individual circumstances and base rates
- Present a fair, balanced analysis""",
        )

        self.defender = CourtAgent(
            role="Risk Analyst B",
            persona="""You are a balanced risk analyst providing a second opinion. You:
- Review Analyst A's assessment for completeness and accuracy
- Identify factors that may have been over- or under-weighted
- Provide your own balanced assessment of the evidence
- Note areas of agreement and disagreement with Analyst A
- Consider alternative interpretations of the same data
- Present a fair, balanced analysis""",
        )

        self.judge = CourtAgent(
            role="Lead Evaluator",
            persona="""You are a lead evaluator synthesizing analyst assessments. You:
- Consider both analysts' assessments equally
- Identify consensus and divergence points
- Weigh the strength of evidence presented by each analyst
- Make a final prediction based on the combined analysis
- Assign confidence based on analyst agreement
- Provide clear reasoning for your decision""",
    ...
    def _build_prompt(self, agent: CourtAgent, turn: int) -> str:
        COURTROOM_RULES = """
1. Assessments must be based on presented evidence and facts
2. Maintain objectivity — consider both risk and protective factors
3. Use professional analytical language
4. Address specific risk factors and protective factors
5. Consider both statistical base rates and individual factors
6. Acknowledge uncertainty where appropriate
7. Build on previous analysts' observations
8. Present clear, structured analysis
9. Cite relevant reasoning for each factor's weight
...
async def main():
    ...
    if args.output_dir:
        ...
        output_base = Path(__file__).resolve().parent.parent / f"transcripts_cooperative_{args.dataset}_{datetime_runstart}"
